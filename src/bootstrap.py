...
def bind(arr):
    ...
    return 1 - np.mean(arr_**4, axis=1) / (3.0 * np.mean(arr_**2, axis=1) ** 2)
...
if __name__ == "__main__":
    ...
    # Input data files
    files = glob(f"raw_data/L{Ns}/**/data.txt", recursive=True)
    ...
    BS_SAMPLES = deepcopy(D_TEMPLATE)
    ...
    for f in files:
        ...
        mean, error, susc_mean, susc_err, binder_mean, binder_error, tau, bs_samples = (
            analyse(polyakov, L, bs_samples=True)
        )
    ...
    # Plot
    for plt_idx, f in enumerate(
        [
            lambda x: np.mean(np.abs(x), axis=1),
            lambda x: 6
            * Ns**3
            * (np.mean(x**2, axis=1) - np.mean(np.abs(x), axis=1) ** 2),
            lambda x: bind(x),
        ]
    ):
        fig, ax = plt.subplots()

        # Plot bootstrap sample means of observables
        x = []
        y = []
        for beta in BETA[Ns]:
            x += len(BS_SAMPLES[Ns][beta]) * [beta]
            y += f(BS_SAMPLES[Ns][beta]).tolist()

        plt.scatter(x, y)

        # Reweight plot
        ax.plot(MRW[Ns][0], MRW[Ns][plt_idx + 1], color=colours[0])

        auto_handles, auto_labels = ax.get_legend_handles_labels()

        legend_elements = [
            Line2D([0], [0], color="k", marker="o", label="data"),
            Line2D([0], [0], color="k", lw=0.5, label="reweight"),
        ]

        ax.legend(handles=legend_elements + auto_handles)
        ax.set_ylabel(symbols[plt_idx])
        ax.set_xlabel(r"$\beta$")

        plt.savefig(output_files[plt_idx])
